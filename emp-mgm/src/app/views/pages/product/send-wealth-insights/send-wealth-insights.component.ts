...
import { sendWealthInsights } from 'src/app/model/package.model';
...
@Component({
  selector: 'app-send-wealth-insights',
  templateUrl: './send-wealth-insights.component.html',
  styleUrls: ['./send-wealth-insights.component.scss']
})
export class SendWealthInsightsComponent implements OnInit {
  today = new Date();
  serverErrorMessages: any;
  isLoading: boolean
  allWealthInsights: Array<sendWealthInsights> = []
  sendResearch: sendWealthInsights = {
    _id: '',
    details: "",
    type: '',
    image: [],
  ...
  async ngOnInit() {
    await this.productService.getSendWealthInsightsCatergory().toPromise()
      .then(res => {
        this.categories = res;
  ...
  refreshList() {
    this.isLoading = true
    this.productService.getAllWealthInsights().toPromise()
      .then(res => {
        this.allWealthInsights = res as sendWealthInsights[];
        this.filter()
        this.isLoading = false
      }).catch(err => { this.isLoading = false; this.alertNotificationService.alertError(err) })
  }

  async onApprove(ann: sendWealthInsights) {
    this.alertNotificationService.alertApprove()
      .then(result => {
        if (result.isConfirmed) {
          ann.approved = true;
          this.productService.approveWealthInsights(ann._id).toPromise()
            .then(res => {
  ...
  onDelete(id) {
    this.alertNotificationService.alertDelete()
      .then(result => {
        if (result.isConfirmed) {
          this.productService.deleteWealthInsights(id).toPromise()
            .then(res => {
    ...
    this.alertNotificationService.alertCustomMsg("Are You sure to proceed furthur?")
      .then(result => {
        if (result.isConfirmed) {
          ...
          this.productService.postSendWealthInsights(this.fd).toPromise()
            .then(res => {
              // sendResearchForm.resetForm();
              // this.sendResearch = new sendWealthInsights();
  ...
  filteredData: sendWealthInsights[] = []
  filter() {
    this.filteredData = [...this.allWealthInsights]
    this.totalSize = this.allWealthInsights.length;
