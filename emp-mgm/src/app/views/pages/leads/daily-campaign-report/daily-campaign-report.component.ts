...
@Component({
  selector: 'app-daily-campaign-report',
  templateUrl: './daily-campaign-report.component.html',
  styleUrls: ['./daily-campaign-report.component.scss']
})
export class DailyCampaignReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.displayedColumns = ['date', 'upload_count', 'same_day_assigned', 'reassigned', 'unassigned', 'unattempted', 'attempted', 'retarget'];
    this.displayedColumns2 = ['date', 'upload_count', 'same_day_assigned', 'reassigned', 'unassigned', 'unattempted', 'attempted', 'retarget'];
  ...
  async submit() {
    this.isLoading = true;
    await this.leadService.displaycampagndailyLeadreports(this.campaignname, this.Lang, this.Level, this.startDate, this.endDate).toPromise()
      .then(res => {
