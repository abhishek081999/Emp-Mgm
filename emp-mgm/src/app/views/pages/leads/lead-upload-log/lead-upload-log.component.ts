...
@Component({
  selector: 'app-lead-upload-log',
  templateUrl: './lead-upload-log.component.html',
  styleUrls: ['./lead-upload-log.component.scss']
})
export class LeadUploadLogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.displayedColumns = ['type', 'by', 'language', 'date','campaign_name', 'lead_count', 'new_addition', 'existing_update', 'retarget', 'skipped'];
    this.refresh()
  }
  ...
  async submit() {
    this.isLoading = true;
    await this.leadService.getLeadUploadLog(this.campaignname, this.Level, this.Lang, this.startDate, this.endDate, this.typeFilter).toPromise()
      .then(res => {
