...
import { Leadstage, Leadreport, getLeadLevel, leadLevel } from 'src/app/model/leads.model';
...
@Component({
  selector: 'app-team-mem-lead-report',
  templateUrl: './team-mem-lead-report.component.html',
  styleUrls: ['./team-mem-lead-report.component.scss']
})
export class TeamMemLeadReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.displayedColumns = ['leadowner', 'leadownername', 'teamlead', 'teamleadname', 'month', 'assigned', 'unattempted', 'attempted', 'retarget', 'demo_given'];
    this.displayedColumns2 = ['leadowner', 'leadownername', 'teamlead', 'teamleadname', 'month', 'assigned', 'unattempted', 'attempted', 'retarget', 'demo_given'];
  ...
  async submit() {
    await this.leadService.displayteammemsLeadreports(this.teamname.name, this.Lang, this.Level).toPromise()
      .then(res => {
    ...
    this.monthslabels = [...new Set(this.displayData.filter(f => f.month).map(e => e.month))]
  ...
  createconvertionchart() {
    if (this.teamname) {
      var convertion = this.displayData.filter(f => f.teamname == this.teamname.name && f.month).map(e => e.Convertion_Rate)
      this.barChartOptions.series = [{ 'name': 'Convertion Rate(%)', data: convertion }]
      var xaxis: ApexXAxis = {
        type: "category",
        categories: this.monthslabels,
        labels: {
          style: {
            colors: '#686868',
            fontSize: '13px',
  ...
  createseatbookedchart() {
    if (this.teamname) {
      var convertion = this.displayData.filter(f => f.teamname == this.teamname.name && f.month).map(e => e.Seat_Booked_Rate)
  ...
  createpiechart() {
    if (this.teamname && this.month) {
      var data = this.displayData.filter((e) => e.teamname == this.teamname.name && e.month == this.month)[0]
      console.log(data)
      this.pieChartOptions2.nonAxisSeries = [data.attempted ? data.attempted : 0, data.unattempted ? data.unattempted : 0]
