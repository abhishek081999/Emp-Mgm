...
@Component({
  selector: 'app-lead-temp-upload',
  templateUrl: './lead-temp-upload.component.html',
  styleUrls: ['./lead-temp-upload.component.scss']
})
export class LeadTempUploadComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.displayedColumns1 = ['select', 'leadowner', 'leadname', 'leadsource', 'leadstatus', 'leadlevel', 'attempt', 'updatedate', 'leadassigndate', 'leaddate', 'callbackdate', 'campaign_name', 'servicename', 'comment', 'whatsappnum', 'city', 'state', 'dob', 'gender', 'occupation', 'experience', 'servicetype', 'servicecode', 'creditcard', 'itrfill', 'monthlyincome', 'logs'];
  ...
  async refreshlist() {
    ...
    await this.leadService.getallLead().toPromise()
      ...
      .catch(err => this.alertNotificationService.alertError(err));
    this.allData = sortNullValues(this.allData, 'leadassigndate', 'date', 'desc') as Leads[];
    this.createData();
  ...
  openfileuploadModal() {
    this.modalService.open(ImportXlsxLeadComponent, { centered: true, scrollable: true, size: 'xl' })
      .result.then((result) => {
        if (result) {
          this.uploadedLeads = [...result]
          this.uploadedLeadsPhone = this.uploadedLeads.filter(f => f.phone).map(e => e.phone)
          this.uploadedLeadsPhone = [...new Set(this.uploadedLeadsPhone)]
          this.uploadedLeadsEmail = this.uploadedLeads.filter(f => f.email).map(e => e.email)
          this.uploadedLeadsEmail = [...new Set(this.uploadedLeadsEmail)]
          this.createData();
        }
      }).catch((res) => { });
  }
  ...
  createData() {
    ...
    if (this.uploadedLeads.length > 0) {
      this.uploadedLeads.forEach(e => {
        var s = [e.whatsappnum, e.phone, e.telegram, e.alternatenumber].filter(k => k);
        var d = this.allData.find(l => (e.email && l.email && e.email == l.email) || s.includes(l.phone) || s.includes(l.whatsappnum) || s.includes(l.telegram) || s.includes(l.alternatenumber))
        if (d) {
          this.displayLeads.push(d)
        } else {
          e.leadstatus = "Not Found";
          this.displayLeads.push(e)
        }
      })
    }
  ...
  leadupload() {
    if (this.filterQuery && this.filterQuery == "not found") {
      var newLeads = [...this.selection.selected];
      newLeads.forEach(e => {
        e.leadstatus = "New"
      });
      ...
      this.alertNotificationService.alertInfo("Please Filter by Not Found Leads");
    }
  ...
  search() {
    this.displayLeads = [];
    this.displayLeads = this.allData.filter(e => e.phone == this.searchLead || e.email == this.searchLead || e.whatsappnum == this.searchLead || e.telegram == this.searchLead || e.alternatenumber == this.searchLead)
    this.tablefilter();
  }
}
