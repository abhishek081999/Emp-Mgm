...
@Component({
  selector: 'app-request-adjustment-history',
  templateUrl: './request-adjustment-history.component.html',
  styleUrls: ['./request-adjustment-history.component.scss']
})
export class RequestAdjustmentHistoryComponent implements OnInit {
  ...
  @Input() data;
  ngOnInit() {
    this.displayedColumns = ["isApproved", "total_amount", "original_amount", "total_discount", "requestedBy", "requestReason", "approvedBy", "comments", "options"];
    this.refreshlist();
  }

  async refreshlist() {
    this.isLoading = true
    await this.orderService.requestAdjustmentforOrder(this.data.orderID).toPromise()
      .then(res => {
