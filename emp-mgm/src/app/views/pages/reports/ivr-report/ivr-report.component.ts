...
export class IVRReportComponent implements OnInit {
  ...
  async ngOnInit() {
    this.displayedColumns = ['team_name', 'team_lead_name', 'staff_name', 'dept_name', 'queueName', 'date_time', 'callType', 'duration', 'answerDuration', 'callCreatedReason', 'endCallCause', 'endedBy',
      'to_alias', 'from_alias', 'option'];
  ...
  filter() {
    this.isLoading = true
    this.reportsServices.getivrreport(this.startDate, this.endDate, this.queryFilter, this.calltypeFilter, this.team, this.employee, this.department).toPromise()
      .then(res => {
  ...
  audioBlobLink
  getAudioLink(link, component) {
    if (link) {
      this.audioBlobLink = link
