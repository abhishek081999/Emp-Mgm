...
@Component({
  selector: 'app-insignia-report-b',
  templateUrl: './insignia-report-b.component.html',
  styleUrls: ['./insignia-report-b.component.scss']
})
export class InsigniaReportBComponent implements OnInit {
  ...
  async ngOnInit() {
    this.displayedColumns = ['month', 'bcmb_active_count', 'bcmb_inactive_count', 'bcmb_count', 'insignia_opted', 'insignia_opted_conv', 'advance_opted', 'advance_opted_conv', 'product_opted', 'product_opted_conv'];
  ...
  async submit() {
    this.isLoading = true
    await this.sellsService.insigniaReport2(this.year).toPromise()
      .then(res => {
  ...
  export() {
    if (this.dataSource) {
      this.exportService.exportonesheet('BCMB to INSG Monthwise Report', this.dataSource.filteredData)
    }
    else {
      this.alertNotificationService.alertInfo('BCMB to INSG Monthwise Report is Empty')
    }
  }
}
