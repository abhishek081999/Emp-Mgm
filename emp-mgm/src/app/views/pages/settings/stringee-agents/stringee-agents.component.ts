...
@Component({
  selector: 'app-stringee-agents',
  templateUrl: './stringee-agents.component.html',
  styleUrls: ['./stringee-agents.component.scss']
})
export class StringeeAgentsComponent implements OnInit {
  ...
  async ngOnInit() {
    this.displayedColumns = ['agentId', 'stringee_id', 'employee_name', 'last_synced', 'department', 'manager_name', 'email', 'option'];
  ...
  filter() {
    ...
    this.settingsServices.getstringeeagentsList(this.startDate, this.endDate, this.DepartmentFilter, this.nameFilter, this.managerFilter).toPromise()
      .then(res => {
  ...
  Sync(component) {
    this.modalService.open(component, { size: 'md', scrollable: true, centered: true, backdrop: 'static' }).result
      .then(resp => {
        this.settingsServices.postsyncstringeeagentschanges(this.data).toPromise()
          .then(res => {
            this.alertNotificationService.toastAlertSuccess('Sync The Changes Added Successfully');
            this.refresh()
          }).catch(err => this.alertNotificationService.alertError(err))
        ...
        this.alertNotificationService.alertCustomMsg('Are you sure you want to Update Employee ?')
          .then(result => {
            if (result.isConfirmed) {
              ...
              this.settingsServices.stringeeagentsupdateemployee(data).toPromise()
                .then(_res => {
